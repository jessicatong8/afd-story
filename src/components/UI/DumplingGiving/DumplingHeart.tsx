...
const DumplingHeart = ({ dropped }: Props) => {
  ...
  const initialPosGlow = "top-[0%] left-[0%]";
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`w-1/7 aspect-square absolute -translate-x-[50%]-translate-y-[50%]
        ${dropped ? droppedPos : initialPos} `}
    >
      <img
        src={dumplingImage}
        className="w-full absolute -translate-x-[50%]-translate-y-[50%] z-1 opacity-100"
      />
      <svg
        // className="border-2"
        className={`w-full h-auto absolute scale-102 z-0
          ${dropped ? "invisible" : initialPosGlow} 
        `}
        width="205"
        height="190"
        viewBox="0 0 205 190"
